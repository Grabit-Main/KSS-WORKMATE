# Employee KPI Scoring Rules & Evaluation Rubric

This document defines the official **KPI Evaluation Rules and Scoring Rubric** for all employees, team leads, project managers, and executive leadership (CEO, CTO). All daily evaluations must strictly adhere to these criteria to ensure fair, transparent, and consistent performance tracking.

---

## 1. Master KPI Scoring Rubric (Scores 1 to 5)

| # | KPI Criteria | 1 – Poor | 2 – Below Standard | 3 – Meets Standard | 4 – Strong | 5 – Exceptional | Weight | Max Contrib. |
|---|--------------|----------|--------------------|--------------------|------------|-----------------|:------:|:------------:|
| **1** | **Task Completion** | <50% committed work completed | 50–79% completed | 80–99% completed | 100% completed | 100% + meaningful additional work | **3** | 15% |
| **2** | **Quality** | Major errors / unusable output | Multiple errors / significant rework | Acceptable output with normal corrections | Very few minor corrections | Excellent output; no significant rework required | **3** | 15% |
| **3** | **Productivity** | Very low useful output | Output significantly below expected | Expected output for available time | Above expected useful output | Significantly above expected output without quality loss | **3** | 15% |
...
## 2. Daily KPI Calculation Formula

The daily performance score is calculated as a weighted percentage:

$$\text{Daily KPI \%} = \sum_{i=1}^{10} (\text{Score}_i \times \text{Weight}_i)$$
...
## 3. Performance Status Tiers

Based on the calculated Daily KPI percentage, the system classifies each evaluation into one of five standardized tiers:
...
## 4. Evaluation Rules & Operational Guidelines

1. **Daily Evaluation Schedule**:
   - Team Leads (TL) evaluate their direct report teammates on every active business day.
   - Scores cannot be submitted for future calendar dates (past and present dates only).
2. **Score Selection Rule**:
   - Evaluators must choose ratings (1–5) based on the exact definitions in the Master Rubric above.
   - Do not arbitrarily assign default high or low scores; refer to the rubric definitions.
3. **Role & Visibility Matrix**:
   - **Team Lead (TL)**: Full permissions to submit and modify daily evaluations for assigned teammates.
   - **Team Member (Employee)**: Can view their own daily KPI scores, percentage breakdowns, and lead feedback notes (read-only).
   - **CEO / CTO / Project Manager (PM)**: Organization-wide read-only visibility into all KPI logs, department averages, summaries, and CSV reports.
