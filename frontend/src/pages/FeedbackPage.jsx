...
const FeedbackPage = () => {
  const [feedbackList, setFeedbackList] = useState(() => {
    const cached = localStorage.getItem('cache_feedback') || localStorage.getItem('cache_reviews');
    return cached ? JSON.parse(cached) : [];
  });
  const [loading, setLoading] = useState(() => !localStorage.getItem('cache_feedback') && !localStorage.getItem('cache_reviews'));
  const { user } = useAuth();
  ...
  const loadFeedback = async () => {
    try {
      ...
      localStorage.setItem('cache_feedback', JSON.stringify(data));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
        ...
        {feedbackList.length === 0 && (
          <div className="card" style={{ gridColumn: '1 / -1', padding: '48px 24px', textAlign: 'center' }}>
            <MessageSquareQuote size={32} strokeWidth={1.5} style={{ margin: '0 auto 12px', display: 'block', color: 'var(--text-tertiary)' }} />
            <h4 className="font-bold text-base mb-1">No Feedback Yet</h4>
