...
async def update_member(team_id: UUID, user_id: UUID, req: SetLeadRequest, db: Session = Depends(get_db), _=Depends(require_pm_up)):
...
async def remove_member(team_id: UUID, user_id: UUID, db: Session = Depends(get_db), _=Depends(require_pm_up)):
    m = db.query(TeamMembership).filter(TeamMembership.team_id == team_id, TeamMembership.user_id == user_id).first()
    if not m:
        raise HTTPException(404, "Member not found")
    db.delete(m)
